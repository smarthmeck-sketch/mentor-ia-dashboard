...
export default function LoginComponent() {
  ...
  const handleOutlookLogin = () => {
    localStorage.setItem('user', JSON.stringify({ email: 'outlook-user@mentor-ia.com', authenticated: true }));
    router.push('/dashboard');
  };
      ...
      <div className="w-full max-w-md">
        ...
        {!showLogin && (
          <>
            ...
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24"><text x="4" y="16" fontSize="14" fill="currentColor">G</text></svg>
              Entrar com Google
            </button>
            <button className="w-full py-3 px-4 bg-slate-700 text-white rounded-lg hover:bg-slate-600 transition-colors font-medium mb-6 flex items-center justify-center gap-2">
              <svg className="w-5 h-5" viewBox="0 0 24 24"><text x="4" y="16" fontSize="14" fill="currentColor">M</text></svg>
              Entrar com Outlook
            </button>
          </>
        )}
